...
init python:
    ...
    class Obstacle(object):
        ...
        def __init__(self, pos, size, velocity, index):
            self.position = pos
            self.size= size
    ...
    class Game1Diasplayable(renpy.Displayable):
        def __init__(self):
            ...
            self.characterPosition = [None, None]
            self.characterPosition[0] = renpy.random.randint(0, int(self.screenSize[0]/5) - int(self.characterSize[0]/2)) + self.screenSize[0]/4
            self.characterPosition[1] = renpy.random.randint(0, int(self.screenSize[1]/5) - int(self.characterSize[1]/2)) + self.screenSize[1]/4
            ...
            self.obstacleVelocity = (3, 0)
            self.obstacles = [None] * self.obstacleNumber

            # 장애물과 다른 객체 "변과 변" 사이의 최소 거리 (0: 서로 변이 붙을 수 있음)
            self.obstacleMargin = (self.obstacleSize[0] * 3, self.obstacleSize[1] * 3)
            ...
            def generatePosition(currentIndex):
                while True:
                    ...
                    # 초기 캐릭터와 위치가 겹치는지 확인
                    if (
                        abs(Position[0] - self.characterPosition[0]) < self.obstacleSize[0]/2 + self.characterSize[0]/2 + self.obstacleMargin[0] and 
                        abs(Position[1] - self.characterPosition[1]) < self.obstacleSize[1]/2 + self.characterSize[1]/2 + self.obstacleMargin[1]
                    ):
                        continue
                    
                    # 초기 도착지점과 위치가 겹치는지 확인
                    elif (
                        abs(Position[0] - self.goalPosition[0]) < self.obstacleSize[0]/2 + self.goalSize[0]/2 + self.obstacleMargin[0] and 
                        abs(Position[1] - self.goalPosition[1]) < self.obstacleSize[1]/2 + self.goalSize[1]/2 + self.obstacleMargin[1]
                    ):
                        continue
                    ...
                    # 다른 장애물이 존재한다면, 서로 위치가 겹치는지 확인
                        ...
                        for index in range(0, currentIndex):
                            otherPos = self.obstacles[index].position

                            if (
                                abs(Position[0] - otherPos[0]) < self.obstacleSize[0] + self.obstacleMargin[0] and 
                                abs(Position[1] - otherPos[1]) < self.obstacleSize[1] + self.obstacleMargin[1]
                            ):
            ...
            for index in range(0, self.obstacleNumber):
                self.obstacles[index] = Obstacle(generatePosition(index), self.obstacleSize, self.obstacleVelocity, index)
            ...
            for obstacle in self.obstacles:
                ...
                self.obstacleImage.append(Image("minigame1_사감쌤.png", pos = (tempXpos - self.obstacleSize[0]/2, tempYpos - self.obstacleSize[1]/2), xsize=self.obstacleSize[0], ysize=self.obstacleSize[1]))
