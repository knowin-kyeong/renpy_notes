...
init python:
    ...
    class Game1Diasplayable(renpy.Displayable):
        def __init__(self, currentmap, lastlocation):
            ...
            def mapUpdate():
                if self.currentmap == 1:
                    ...
                    self.map = [[[700,300],[25,82],[0,0],0,"minigame1_사감쌤.png"],
                                [[400,500],[25,82],[0,0],0,"minigame1_사감쌤.png"]]

                elif self.currentmap == 2:
                    self.goalSize= (40, 100)
                    self.goalPosition = [100, 100]
                    self.map = [[[30,30],[50,101],[7,7],1,"minigame1_교장쌤.png"],
                                [[170,170],[50,101],[7,7],1,"minigame1_교장쌤.png"],
                                [[200,200],[25,82],[0,0],0,"minigame1_사감쌤.png"],
                                [[400,800],[25,82],[0,0],0,"minigame1_사감쌤.png"],
                                [[800,400],[25,82],[0,0],0,"minigame1_사감쌤.png"]]
        ...
        def event(self, ev, x, y, st):
            ...
            if currentmap ==0:
                if mousePress and abs(mousePosition[0] - self.goalPosition[0] - self.goalSize[0]/2) < (self.goalSize[0]/2) and abs(mousePosition[1] - self.goalPosition[1] - self.goalSize[1]/2) < (self.goalSize[1]/2):
                    self.endedByReachingGoal = True
                    self.lastlocation = [list(mousePosition)[0]-self.characterSize[0]/2,list(mousePosition)[1]-self.characterSize[1]/2]
                ...
                velocityVector = [mousePosition[0] - self.characterPosition[0] - self.characterSize[0]/2, mousePosition[1] - self.characterPosition[1]- self.characterSize[1]/2]
                velocityNorm =  math.sqrt((velocityVector[0])**2 + (velocityVector[1])**2)
                ...
                def is_collided():
                    for obstacle in self.obstacles:
                        if (
                            abs(self.characterPosition[0] - obstacle.position[0]) < (self.characterSize[0]/2 + obstacle.size[0]/2) and
                            abs(self.characterPosition[1] - obstacle.position[1]) < (self.characterSize[1]/2 + obstacle.size[1]/2)
                        ):
                            return True
                    return False
